...
def ExportResult(run_from, run_days, calc_price, result, file_path):
    export_days = run_days
    if export_days > 255: # Excel 最大 256 列，首列显示价格，剩余可用 255 列
        export_days = 255
        print("提示：Excel 最大 256 列，剩余 %d 列数据未作导出！" % (run_days - 255))
    df_result = pd.DataFrame(result[:, run_from : (run_from + export_days)]).iloc[::-1] # 上下倒下顺序
    df_result.index = calc_price[::-1]
    df_result.columns = ["day_%d" % (days + 1) for days in np.arange(run_from, run_from + export_days, 1)]
...
def Test_DerivX_Barrier_Single():
    ...
    for name, flag in greek_flags.items():
        ...
        ExportResult(run_from, run_days, calc_price, np.array(greeks), "/export_greeks_%s.xls" % name)
