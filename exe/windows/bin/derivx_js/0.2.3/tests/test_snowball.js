...
class Config {
    constructor() {
        this.runs_seed = false // 随机路径种子 // InitPath
...
function Test_Snowball() {
    ...
    config.runs_seed = false // 随机路径种子 // InitPath
    ...
    let path_file = util.format("./path_data_%d_%d_%d_%d_%d_%f_%f_%f_%f.path", 
        config.runs_seed, config.dual_smooth, config.runs_size, config.runs_step, config.year_days, 
        config.sigma, config.risk_free_rate, config.basis_rate, config.price_limit_ratio)
